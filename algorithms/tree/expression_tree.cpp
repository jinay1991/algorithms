...
void ExpressionTree::Construct(const std::string& expression)
{
    ...
    for (auto i = 0; i < expression.length(); ++i)
    {
        ...
        {
            while (!operator_stack.empty() && ((expression[i] != '^' && GetOperatorPrecedence(operator_stack.top()) >=
                                                                            GetOperatorPrecedence(expression[i])) ||
                                               (expression[i] == '^' && GetOperatorPrecedence(operator_stack.top()) >
                                                                            GetOperatorPrecedence(expression[i]))))
            {
                auto node_2 = node_stack.top();
                node_stack.pop();
...
double ExpressionTree::EvaluateExpression() const
{
...
std::string ExpressionTree::GetInfixExpression() const
{
    return root_->Inorder();
}
std::string ExpressionTree::GetPostfixExpression() const
{
    return root_->Postorder();
}
std::string ExpressionTree::GetPrefixExpression() const
{
    return root_->Preorder();
}
