...
TEST(SimpleGraph, AddEdge_GivenTypicalInputs_ExpectUpdatedEdge)
{
    ...
    ASSERT_THAT(simple_graph.GetNeighbours('a'), ::testing::IsEmpty());

    // When
    simple_graph.AddEdge('a', {'b'});

    // Then
    EXPECT_THAT(simple_graph.GetNeighbours('a'), ::testing::ElementsAre('a', 'b'));
    EXPECT_THAT(simple_graph.GetNeighbours('b'), ::testing::IsEmpty());
}
